...
function RecordedHistory() {
  ...
  const backendURL = "http://localhost:5000"; // Update if backend is hosted elsewhere
        ...
        {recordedVideos.length > 0 ? (
          recordedVideos.map((video, index) => (
            <div
              ...
              onClick={() => {
                // Correctly construct the video URL
                const fullUrl = `${backendURL}/videos/${video.filename}`;
                console.log("Selected video:", fullUrl);
                setSelectedVideoUrl(fullUrl);
                setPlaybackError(null); // Reset error on new selection
              }}
            >
